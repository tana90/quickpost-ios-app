...
class BeautifulBarChart: UIView {
    ...
    private let topBubbleRadius: CGFloat = 15.0
    ...
    private func showEntry(index: Int, entry: BarEntry) {
        ...
        barWidth = frame.size.width / (CGFloat(dataEntries?.count ?? 1) / 1.75)
        ...
        var bubleColor = entry.color
        if index == selectedIndex {
            bubleColor = UIColor.black
        }
        /// Draw the top bubble
        drawTopBuble(xPos: xPos+barWidth/2-topBubbleRadius, yPos: round(yPos - height - 80) + 5, color: bubleColor)
        ...
        drawTextValue(xPos: xPos+barWidth/2-topBubbleRadius, yPos: (yPos - height - 86) + 5, textValue: entry.textValue)
        
        /// Draw text below the bar
        drawTitle(xPos: xPos, yPos: yPos + 10, title: entry.title, color: entry.color, current: index == 0 ? true : false)
    }
    ...
    private func drawBar(xPos: CGFloat, yPos: CGFloat, height: CGFloat, color: UIColor) {
        ...
        rightLine.shadowColor = UIColor.black.cgColor
        rightLine.shadowRadius = 5.0
        rightLine.shadowOpacity = 0.1
        rightLine.shadowOffset = CGSize(width: 3, height: 1)
    ...
    private func drawTitle(xPos: CGFloat, yPos: CGFloat, title: String, color: UIColor, current: Bool) {
        let textLayer = CATextLayer()
        textLayer.frame = CGRect(x: xPos, y: yPos, width: barWidth, height: 22)
        textLayer.foregroundColor = UIColor.black.cgColor//color.cgColor
        textLayer.backgroundColor = UIColor.clear.cgColor
        textLayer.alignmentMode = kCAAlignmentCenter
        textLayer.contentsScale = UIScreen.main.scale
        if current {
            textLayer.font = CTFontCreateWithName(UIFont.systemFont(ofSize: 0, weight: .bold).fontName as CFString, 0, nil)
        } else {
            textLayer.font = CTFontCreateWithName(UIFont.systemFont(ofSize: 0).fontName as CFString, 0, nil)
        }
    ...
    private func drawLinkingLine(xPos: CGFloat, yPos: CGFloat, color: UIColor) {
        ...
        line.strokeColor = UIColor.black.cgColor//color.cgColor
        line.lineWidth = 0.5
